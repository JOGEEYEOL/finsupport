import { collection, getDocs, query, where, doc, updateDoc, onSnapshot, orderBy } from "https://www.gstatic.com/firebasejs/11.8.0/firebase-firestore.js";
...
window.login = async function() {
  ...
  if (!code || !password) {
    showError(errorDiv, '담당자 코드(또는 가이아 아이디)와 비밀번호를 입력해주세요.');
    return;
  }
...
window.editField = function(fieldName) {
  ...
  if (fieldName === 'role') {
    ...
    if (parts.length >= 2) {
      carrierValue = parts[0];
      const phoneNumber = parts.slice(1).join(' ');
      [phone1, phone2, phone3] = StringUtils.splitPhoneNumber(phoneNumber);
    } else {
      [phone1, phone2, phone3] = StringUtils.splitPhoneNumber(currentPhone);
    }
    
    // 동적으로 phone 입력 HTML 생성
    phoneGroup.innerHTML = `
      <select id="edit-modal-carrier">
        <option value="">통신사 선택</option>
        <option value="SKT"${carrierValue === 'SKT' ? ' selected' : ''}>SKT</option>
        <option value="KT"${carrierValue === 'KT' ? ' selected' : ''}>KT</option>
        <option value="LGU"${carrierValue === 'LGU' ? ' selected' : ''}>LG U+</option>
        <option value="SKT알뜰"${carrierValue === 'SKT알뜰' ? ' selected' : ''}>SKT 알뜰폰</option>
        <option value="KT알뜰"${carrierValue === 'KT알뜰' ? ' selected' : ''}>KT 알뜰폰</option>
        <option value="LGU알뜰"${carrierValue === 'LGU알뜰' ? ' selected' : ''}>LG U+ 알뜰폰</option>
      </select>
      <div class="phone-number-group">
        <input type="text" class="editClientInfoInputPhone first" id="edit-modal-phone1" value="${phone1}" maxlength="3" />
        <span class="phone-separator">-</span>
        <input type="text" class="editClientInfoInputPhone middle" id="edit-modal-phone2" value="${phone2}" maxlength="4" />
        <span class="phone-separator">-</span>
        <input type="text" class="editClientInfoInputPhone last" id="edit-modal-phone3" value="${phone3}" maxlength="4" />
      </div>
    `;
    ...
    // 새로 생성된 요소들 참조
    const carrierSelect = document.getElementById('edit-modal-carrier');
    const phone1Input = document.getElementById('edit-modal-phone1');
    const phone2Input = document.getElementById('edit-modal-phone2');
    const phone3Input = document.getElementById('edit-modal-phone3');
    
    carrierSelect.focus();
    
    // 숫자만 입력 및 자동 이동 이벤트 리스너 추가
    [phone1Input, phone2Input, phone3Input].forEach((input, index) => {
      input.addEventListener('input', function(e) {
        ...
        if (e.target.value.length >= e.target.maxLength) {
          if (index < 2) {
            const nextInput = [phone1Input, phone2Input, phone3Input][index + 1];
            if (nextInput) {
              nextInput.focus();
              nextInput.select();
            }
      ...
      input.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Backspace' && e.target.value === '' && index > 0) {
          const prevInput = [phone1Input, phone2Input, phone3Input][index - 1];
          if (prevInput) {
            prevInput.focus();
...
window.resetPassword = async function() {
  ...
  if (!code) {
    showAlert('담당자 코드를 입력해주세요.');
    return;
  }
  
  showConfirmDialog('비밀번호를 초기화하시겠습니까?\n\n초기화 후 비밀번호: 0000\n이 비밀번호로 로그인 후 새로운 비밀번호로 변경해주세요.', async () => {
    await performPasswordReset(code);
  });
}

async function performPasswordReset(code) {
